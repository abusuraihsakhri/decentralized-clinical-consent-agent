"""
Command-Line Interface for ConsentLedger: Smart Contract Dynamic Patient Data Use Authorization Agent.
"""
import argparse
import csv
import json
...
from .agents import ConsentLedgerCoordinator

coordinator = ConsentLedgerCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="decentralized-clinical-consent-agent", description="ConsentLedger: Smart Contract Dynamic Patient Data Use Authorization Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  CONSENTLEDGER: SMART CONTRACT DYNAMIC PATIENT DATA USE AUTHORIZATION AGENT")
        print(f"  Domain: Privacy-Preserving Federated Healthcare & FHE | Standard: HL7 FHIR Consent Resource & W3C DID")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting ConsentLedger: Smart Contract Dynamic Patient Data Use Authorization Agent on http://{args.host}:{args.port}")
